...
%Ng
%Nvc
NgType=NgType_parametro; % NgType=1/2 for cyclic prefix/zero padding
if NgType==1, nt='CP'; elseif NgType==2, nt='ZP'; end
Ch=Ch_parametro; % Ch=0/1 for AWGN/multipath channel
if Ch==0, chType='AWGN'; Target_neb=100; else chType='CH'; Target_neb=500; end
%figure(Ch+1), clf
%PowerdB=[0 -8 -17 -21 -25]; % Channel tap power profile ’dB’
PowerdB=[0 -3 -5 -7 -9]; % Channel tap power profile ’dB’

Delay=[0 5 7 8 10];% Channel delay ’sample’
...
Nbps=Nbps_parametro; M=2^Nbps;% Modulation order=2/4/6 for QPSK/16QAM/64QAM
Nfft=Nfft_parametro;% FFT size
Ng=Ng_parametro;% GI (Guard Interval) length (Ng=0 for no GI)
...
Nused=Nfft-Nvc;
EbN0=[0:5:30];% EbN0
N_iter=1e4;% Number of iterations for each EbN0
...
randn('state',0); rand('state',0); %Ber2=ber(); % BER initialization
    ...
    for m=1:N_iter
        ...
        %X= randi(M,Nused*Nframe,1); % bit: integer vector
